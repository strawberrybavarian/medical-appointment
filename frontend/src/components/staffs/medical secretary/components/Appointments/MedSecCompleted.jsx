...
const MedSecCompleted = ({ allAppointments, setAllAppointments }) => {
  const { did } = useParams();
  ...
  const filteredAppointments = sortedAppointments
    ...
    .filter(appointment => {
        // Apply date filter logic if both start and end dates are provided
        if (dateFilterStart && dateFilterEnd) {
          const appointmentDate = new Date(appointment.date);
          return appointmentDate >= new Date(dateFilterStart) && appointmentDate <= new Date(dateFilterEnd);
        }
        return true; // If no date filter, return all appointments
    });
  ...
  const exportToCSV = () => {
    const headers = [
      "Patient Name", "Doctor Name", "Service", "Date", "Time", "Appointment Status", "Follow Up"
    ];
  
    const rows = filteredAppointments.map(appointment => [
      `${appointment.patient?.patient_firstName} ${appointment.patient?.patient_middleInitial}. ${appointment.patient?.patient_lastName}`,
      appointment.doctor ? `${appointment.doctor.dr_firstName} ${appointment.doctor.dr_lastName}` : "Not Assigned",
      appointment.appointment_type.map(type => type.appointment_type).join(', '),
      appointment.date ? new Date(appointment.date).toLocaleDateString() : "Not Assigned",
    ...
    // Get the start and end dates from the filtered appointments
    const dates = filteredAppointments.map(appointment => new Date(appointment.date));
    const startDate = moment(Math.min(...dates)).format('YYYY-MM-DD');
    const endDate = moment(Math.max(...dates)).format('YYYY-MM-DD');
  
    // Trigger download
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    saveAs(blob, `${startDate}_${endDate}_completed_appointments.csv`);
  };
      ...
      <Container fluid className="w-100">
          ...
          <div >
            <Row>
              ...
              <Form.Group controlId="formSearch" className="d-flex align-items-center">
                <Form.Label style={{  marginLeft: '1vh' }}>Appointment Date:</Form.Label>
                <Row>
                  <Col>
                    <Form.Control
                      type="date"
              ...
              {currentAppointments.map((appointment) => {
                ...
                return (
                  <tr key={appointment._id}>
                    ...
                    <td>
                      <div className="">
                      <div className="completed-appointment" style={{fontSize: '12px'}}>
                            {appointment.status}
                      </div>
                      </div>
                    </td>
...
export default MedSecCompleted;
