...
function Prescription({ patientId, appointmentId, doctorId }) {
  ...
  const addMedication = () => {
    if (medication.name && medication.type && medication.instruction) {
      setMedications(prevMedications => [...prevMedications, medication]); // Ensure you use the functional state update form
  ...
  const handleSubmit = async () => {
    ...
    formData.append('medications', JSON.stringify(medications)); // Ensure proper serialization
    if (rximage) {
  ...
  return (
    <>
      <Container className="prescription-main">
        <h1>Create Prescription</h1>
        <p>Appointment ID: {appointmentId}</p>

        <Container className="prescription-form">
          <Container className="prescription-preview">
            <Form>
              <Row>
                <Col>
        ...
        <Container className="prescription-previewmain">
